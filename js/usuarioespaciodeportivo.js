...
document.getElementById("btn-consultar").addEventListener("click", function(event) {
    ...
    const UED_Id = document.getElementById("UED_Id").value;

    // Realiza una solicitud GET a la API para obtener los datos correspondientes al ID
    fetch(`http://localhost:3000/api/usuarioespaciodeportivo/${UED_Id}`)
        .then(response => {
            if (!response.ok) {
                document.getElementById("UED_Uio_Id").value = "";
                document.getElementById("UED_ED_Id").value = "";
        ...
        .then(data => {
            ...
            document.getElementById("UED_Uio_Id").value = data.UED_Uio_Id;
            document.getElementById("UED_ED_Id").value = data.UED_ED_Id;
        })
        .catch(error => {
